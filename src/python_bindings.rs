...
impl StateHandler for PythonHandler {
    async fn enter(&self, context: &mut StateContext) -> Result<State> {
        Python::with_gil(|py| {
            ...
            let result = self.py_handler.call_method1(py, "enter", (context_py,))
                .map_err(|e| ConnectError::StateMachine(format!("Python handler error: {}", e)))?;
            let state_str: String = result.extract(py)
                .map_err(|e| ConnectError::StateMachine(format!("Failed to extract state: {}", e)))?;
    ...
    async fn execute(&self, context: &mut StateContext) -> Result<State> {
        Python::with_gil(|py| {
            let context_py = PyStateContext::new(context);
            let result = self.py_handler.call_method1(py, "execute", (context_py,))
                .map_err(|e| ConnectError::StateMachine(format!("Python handler error: {}", e)))?;
            let state_str: String = result.extract(py)
                .map_err(|e| ConnectError::StateMachine(format!("Failed to extract state: {}", e)))?;
            Ok(parse_state_string(&state_str))
    ...
    async fn exit(&self, context: &mut StateContext) -> Result<()> {
        Python::with_gil(|py| {
            let context_py = PyStateContext::new(context);
            self.py_handler.call_method1(py, "exit", (context_py,))
                .map_err(|e| ConnectError::StateMachine(format!("Python handler error: {}", e)))?;
            Ok(())
        })
    }
...
impl PyConnection {
    ...
    pub fn execute_query(&self, query: String) -> PyResult<Vec<PyObject>> {
        Python::with_gil(|py| {
            let mut conn_guard = self.inner.try_lock().map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
            })?;
            
            let results: Vec<mysql::Row> = conn_guard.exec(&query, ()).map_err(|e| {
                PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(e.to_string())
            })?;
                    ...
                    let py_value = match value {
                        ...
                        mysql::Value::Date(year, month, day, hour, minute, second, _) => {
                            format!("{}-{:02}-{:02} {:02}:{:02}:{:02}", 
                                   year, month, day, hour, minute, second).into_py(py)
                        }
