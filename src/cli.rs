...
pub type ConnInfoResult = std::result::Result<(String, String, String, Option<String>), Box<dyn std::error::Error>>;
...
impl CommonArgs {
    ...
    pub fn load_config(&self) -> Result<AppConfig> {
        if let Some(ref config_path) = self.config {
            ...
            let default_paths = ["tidb_config.json", "tidb_config.toml", "config/tidb.json", "config/tidb.toml"];
    ...
    pub fn get_connection_info_from_config(&self) -> Result<(String, String, String, Option<String>)> {
        ...
        let password = if let Some(pwd) = merged_config.get_password() {
            ...
            return Err(crate::errors::ConnectError::Configuration("No password available".to_string()));
        };
    ...
    pub fn init_logging_from_config(&self) -> Result<()> {
        ...
        if let Some(ref file_path) = merged_config.logging.file {
            log_config = log_config.with_file(true).with_file_path(PathBuf::from(file_path));
        }
