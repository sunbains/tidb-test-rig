...
use test_rig::{
    ConfigBuilder, apply_extensions_to_command, apply_extensions_to_config, print_extensions_help,
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "  cargo run --bin job_monitor --features import_jobs -- -c {}",
        output_path.display()
    );
    println!(
        "  cargo run --bin isolation --features isolation_test -- -c {}",
        output_path.display()
    );
